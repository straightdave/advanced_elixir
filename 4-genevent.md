...
加上这个处理器之后，可以看到，调用了```sync_notify/2```之后，```Forwarder```处理器成功地把事件转给了我们的收件箱（inbox）。

这里有几点需要注意：  

1. 事件处理器运行在事件管理器同一个进程里
2. ```sync_notify/2```同步地运行事件处理器处理请求
3. ```notify/2```使事件处理器异步处理请求

因此，```sync_notify/2```和```notify/2```类似于GenServer里面的```call/2```和```cast/2```。推荐使用```sync_notify/2```。
...
为了能发出事件消息，我们要稍微修改一下我们的注册表，使之同一个事件管理器协作。
